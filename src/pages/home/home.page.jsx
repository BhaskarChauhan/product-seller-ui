...
const HomePage = () => {
  ...
  const [startTime] = useState(currentUser?.createTime);
  const [milliSecElapsed, setMilliSecElapsed] = useState(0);
  useEffect(() => {
    const timerInterval = setInterval(() => {
      const startTimeInMilliseconds = new Date(startTime).getTime();
      const currentTimeInMilliseconds = Date.now();
      const elapsed = currentTimeInMilliseconds - startTimeInMilliseconds;
      setMilliSecElapsed(elapsed);
    }, 1);
  ...
  //console.log(milliSecElapsed);
  if (milliSecElapsed === 10000) {
